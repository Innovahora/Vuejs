...
export default {
    name: 'graphicApvWithOutPensionEndColumn',
    components: {
	  	highcharts: Chart 
  	},
    data () {
	    return {
	      	chartOptions: {
			    ...
			    title: {
			        text: 'Evolución de aportes voluntarios sin fin previsional por AFP'
			    },
			    subtitle: {
			        text: '(En miles de S/)'
			    },
  	...
  	watch: {
  		get_apv_with_out_pension_end() {
  			...
  			if (Object.keys(self.get_apv_with_out_pension_end).length > 0 && self.get_apv_with_out_pension_end != null) {
  				...
  				$.each(self.get_apv_with_out_pension_end.graphic_apv_pension_without_purpose_bar, function(index, val) {
  					if (index == 'dates') {
  				...
  				self.chartOptions.series = [{
			        name: 'Habitat',
			        data: self.get_apv_with_out_pension_end.graphic_apv_pension_without_purpose_bar.habitat.values
			    }, {
			        name: 'Integra',
			        data: self.get_apv_with_out_pension_end.graphic_apv_pension_without_purpose_bar.integra.values
			    }, {
			        name: 'Prima',
			        data: self.get_apv_with_out_pension_end.graphic_apv_pension_without_purpose_bar.prima.values
			    }, {
			        name: 'Profuturo',
			        data: self.get_apv_with_out_pension_end.graphic_apv_pension_without_purpose_bar.profuturo.values
			    }];
  			}
  		}
  	},
